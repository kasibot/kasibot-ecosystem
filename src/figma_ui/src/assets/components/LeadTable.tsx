...
export function LeadTable({ leads, crmConnected = false }: LeadTableProps) {
  ...
  const filteredLeads = leads.filter(lead => {
    ...
    if (filter === 'hot') return lead.estimatedValue > 5000;
    if (filter === 'followup') return lead.followUpRequired;
    if (filter === 'missed') return lead.outcome === 'Missed';
    if (filter === 'new') return lead.status === 'In Progress';
    return true;
  });
